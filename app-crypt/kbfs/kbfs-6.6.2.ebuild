...
src_compile() {
	...
	ego build -v -ldflags='-s -w' -tags production -o "${T}/kbfsfuse"
	popd || die
	pushd ./go/kbfs/kbfsgit/git-remote-keybase || die
	ego build -v -ldflags='-s -w' -tags production -o "${T}/git-remote-keybase"
	popd || die
	pushd ./go/kbfs/redirector || die
	ego build -v -ldflags='-s -w' -tags production -o "${T}/keybase-redirector"
	popd || die
}
